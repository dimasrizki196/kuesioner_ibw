...
export default function Step3Narasi() {
  ...
  // STATE: Mengecek persetujuan baca
  ...
  useEffect(() => {
    ...
    // 3. TENTUKAN FILE PDF (Pastikan path dan casing di folder public sudah sesuai)
    if (type === "politik") {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 md:p-8 overflow-x-hidden selection:bg-blue-500/20 font-serif">
      ...
      {/* CONTAINER UTAMA */}
      <div className="relative z-10 w-full max-w-5xl h-full md:h-[85vh] my-auto animate-in fade-in zoom-in-95 duration-700 p-1">
          ...
          <div className="w-full md:w-[35%] bg-[#080c1d]/90 p-8 md:p-12 text-slate-100 flex flex-col justify-between shrink-0 border-b md:border-b-0 md:border-r border-slate-800/80">
            ...
            <div className="mt-8 space-y-4 pt-6 md:pt-8 border-t border-slate-800/60">
              ...
              >
                ...
                <span>Membaca Lebih Lega (Layar Penuh)</span>
              </button>
            </div>
          </div>

          {/* --- MAIN CONTENT AREA (PDF VIEWER DENGAN PINCH-TO-ZOOM NATIVE) --- */}
          <div className="flex-1 flex flex-col min-h-0 bg-transparent">
            <div className="flex-1 p-3 md:p-10 flex flex-col items-center justify-center relative overflow-hidden">
              {pdfPath ? (
                /* KUNCI FITUR MOBILE: Wadah ini harus dikonfigurasi agar Browser Mobile 
                  mengaktifkan gesture zoom dan scroll di dalamnya.
                  - overflow-auto: Memungkinkan scroll atas-bawah dan kanan-kiri.
                  - touch-auto: Memberitahu iOS/Android: "Biarkan jari mengontrol elemen ini."
                  - selection:none: Mencegah teks terblok saat cubit layar.
                */
                <div className="w-full h-full rounded-[24px] md:rounded-[32px] border border-slate-700/80 shadow-[0_0_30px_rgba(0,0,0,0.5)] bg-slate-100 relative overflow-auto touch-auto selection:none custom-scrollbar">
                  {/* Menggunakan object tag lebih stabil untuk native touch gesture di mobile dibanding iframe */}
                  <object
                    data={`${pdfPath}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`}
                    type="application/pdf"
                    className="w-full h-full border-none"
                    // Fallback jika browser sangat kuno
                  >
                    <div className="w-full h-full flex flex-col items-center justify-center p-10 text-center gap-4 bg-slate-200">
                      <svg
                      ...
                      >
                        Klik di sini untuk mengunduh narasi.
                      </a>
                    </div>
                  </object>
                </div>
      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-8 animate-in fade-in zoom-in-95 duration-300">
          <div
            className="absolute inset-0 bg-black/80 backdrop-blur-md cursor-pointer"
            onClick={() => setShowPopup(false)}
          ></div>
          <div className="relative bg-slate-900 border border-slate-700 w-full max-w-5xl h-[95vh] rounded-[32px] shadow-[0_0_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden">
            <div className="p-4 md:p-5 border-b border-slate-800 flex justify-between items-center bg-slate-950 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-sans font-black text-slate-400 uppercase tracking-widest">
                  MODE LAYAR PENUH (Gunakan 2 Jari untuk Zoom)
                </p>
              </div>
              <button
                onClick={() => setShowPopup(false)}
            ...
            {/* Wadah di Pop-up juga diaktifkan Gesture Zoom-nya */}
            <div className="flex-1 w-full bg-slate-100 overflow-auto touch-auto selection:none custom-scrollbar">
              <object
                data={`${pdfPath}#toolbar=1`} // Aktifkan toolbar di fullscreen untuk kontrol tambahan
                type="application/pdf"
                className="w-full h-full border-none"
              />
            </div>
            <div className="p-4 border-t border-slate-800 text-center bg-slate-950 shrink-0">
              <button
                onClick={() => setShowPopup(false)}
                className="w-full md:w-auto px-10 py-3.5 bg-blue-600 text-white rounded-2xl font-sans font-black uppercase tracking-widest text-[10px] shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:bg-blue-500 active:scale-95 transition-all"
              >
      ...
      {/* Tambahan Style untuk Scrollbar di PDF Viewer Container */}
      <style jsx global>{`
